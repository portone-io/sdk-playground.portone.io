  ...
  var requestIdForPayport = 'requestIdForPayport';
	...
	var Util = (function() {
		return {
			injectQuery : function(url, query) {
				...
				return anchor.protocol + '//' + anchor.host + anchor.pathname + search + anchor.hash;
			}
		}
	})();
	...
	var UserAgent = (function() {
		...
		function _populate() {
			...
			var agent = /(?:MSIE.(\d+\.\d+))|(?:(?:Firefox|GranParadiso|Iceweasel).(\d+\.\d+))|(?:Opera(?:.+Version.|.)(\d+\.\d+))|(?:AppleWebKit.(\d+(?:\.\d+)?))|(?:Trident\/\d+\.\d+.*rv:(\d+\.\d+))/
				.exec(uas);
    ...
    var defaultEnMessage = 'To enable the Paypal payment window, please disable "Block pop-ups" in browser settings';
    var defaultMessage = defaultLang === 'ko' ? defaultKoMessage : defaultEnMessage;
	...
	var SHINHAN = (function() {
		...
		ShinhanPG.prototype.open = function(request_id, merchant_uid) {
			this.popup = window.open('about:blank', this.targetName, 'height=400,width=640,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
			if ( this.popup ) {
				...
				function detectSelfClosed(popup) {
					...
					return setTimeout(function() {detectSelfClosed(popup);}, 100);
				}
	...
	var NAVERCO = (function() {
		...
		NaverCheckout.prototype.open = function(request_id, merchant_uid, popupMode) {
	...
	var NAVERPAY = (function() {
		...
		NaverPay.prototype.open = function(requestId, merchantUid, popupMode, v2) {
			...
			this.popup = window.open('about:blank', "IMP_NAVERPAY", 'height=900,width=760,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
			if ( this.popup ) {
				var that = this;
				...
				function detectSelfClosed(popup) {
					...
					return setTimeout(function() {detectSelfClosed(popup);}, 50);
				}
		...
		NaverPay.prototype.openLayer = function(proxyData) {
			...
			function _open(_proxyData) {
				...
				if ( typeof _proxyData.extraDeduction == 'boolean' )	npayParam.extraDeduction = _proxyData.extraDeduction;
			...
			if ( self.npay ) {
				...
				jQuery.getScript("https://nsp.pay.naver.com/sdk/js/naverpay.min.js", function() {
					self.npay = Naver.Pay
	...
	var PAYCO = (function() {
		...
		Payco.prototype.open = function(requestId, merchantUid) {
			this.popup = window.open('', "IMP_PAYCO", 'top=100, left=300, width=727px, height=512px, resizble=no, scrollbars=yes');
			if ( this.popup ) {
				var that = this;
	...
	var DANALCERT = (function() {
		...
		DanalCert.prototype.open = function(request_id, merchant_uid, popupMode) {
			...
			this.popup = window.open('about:blank', this.targetName, 'height=800,width=440,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
	...
	var INICISCERT = (function() {
		...
		InicisCert.prototype.open = function(request_id, merchant_uid, popupMode) {
			...
			this.popup = window.open('about:blank', this.targetName, 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=450,height=750,top=100,left=100');
	...
	var INICISUNIFIEDCERT = (function() {
		...
		InicisUnifiedCert.prototype.open = function(request_id, merchant_uid, popupMode) {
			if ( UserAgent.mobile() && !popupMode )	return;
	...
	var SettleFirm = (function() {
		...
		SettleFirm.prototype.open = function(requestId, merchantUid, popupMode, _currentIframe) {
			if (!popupMode) {
				jQuery(_currentIframe.iframe).addClass('layer');
				return;
			}

			this.popup = window.open('', "IMP_SETTLE_FIRM", 'top=100, left=300, width=480px, height=770px, resizble=no, scrollbars=yes');
			if ( this.popup ) {
				var that = this;
	...
	var NAVERZZIM = (function() {
		...
		NaverZzim.prototype.open = function(request_id, merchant_uid) {
			...
			this.popup = window.open('about:blank', "IMP_NAVER_ZZIM", 'height=600,width=480,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
		}
	...
	var PAYPALMODAL = (function() {
		...
		PAYPALMODAL.prototype.open = function(requestId, merchantUid, mode) {
			this.mode = mode;
			if ( this.mode === true ) {
				this.popup = window.open('about:blank', "IMP_PAYPAL_MODAL", "top=100, left=300, width=440px, height=700px, resizble=no, scrollbars=yes");
				if ( this.popup ) {
					var that = this;
	...
	var EXIMBAY = (function() {
		...
		EximbayPG.prototype.open = function(request_id, merchant_uid, popup_mode) {
			...
			this.popup = window.open('about:blank', this.targetName, 'top=100,left=400,height=400,width=640,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
			if ( this.popup ) {
	...
	var ChaiPay = (function() {
		...
		ChaiPay.prototype.open = function(requestId, merchantUid, popupMode) {
			...
			this.popup = window.open('about:blank', this.targetName, 'top=100,left=400,height=740,width=540,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
			if ( this.popup ) {
				this.monitoring = true;
				var that = this;
	...
	var Smartro = (function() {
		...
		Smartro.prototype.open = function(requestId, merchantUid) {
			this.popup = window.open('about:blank', this.targetName, 'width=620,height=405,left=150,top=150,toolbar=no,location=no,directories=no,status=yes,menubar=no,status=yes,menubar=no,scrollbars=no,resizable=yes');
			if ( this.popup ) {
				this.monitoring = true;
				var that = this;
	...
	var Tosspay = (function() {
		...
		Tosspay.prototype.open = function(requestId, merchantUid) {
			this.popup = window.open('about:blank', this.targetName, 'width=460,height=670,left=150,top=150,toolbar=no,location=no,directories=no,status=yes,menubar=no,status=yes,menubar=no,scrollbars=no,resizable=yes');
			if ( this.popup ) {
				this.monitoring = true;
				var that = this;
	...
	var KcpQuick = (function() {
		...
		KcpQuick.prototype.open = function(requestId, merchantUid) {
			this.popup = window.open('about:blank', this.targetName, 'width=480,height=720,left=150,top=150,toolbar=no,location=no,directories=no,status=yes,menubar=no,status=yes,menubar=no,scrollbars=no,resizable=yes');
			if ( this.popup ) {
				this.monitoring = true;
				var that = this;
	...
	var XDM = (function(window) {
		...
		var getFrameInst = (function() {
            ...
            paymentResult = {};
					...
					if ( origin !== api_server && origin !== payportFrontDomain)	return false;
            ...
            paymentResult = obj.paymentResult;
					...
					if ( action === 'kakao.dlp' ) { //mobile ios전용
						...
						d.then(function() {
							...
							kakaopayDlp.setChannelType(proxyData.channel.key, proxyData.channel.value);
							kakaopayDlp.addRequestParams(proxyData.param); // 초기값 세팅

							if ( proxyData.returnUrl )	kakaopayDlp.setReturnUrl(proxyData.returnUrl);
							if ( proxyData.cancelUrl )	kakaopayDlp.setCancelUrl(proxyData.cancelUrl);

							kakaopayDlp.callDlp('kakaopay_layer', document.forms['kakaoPayFormProxy'], function(data) {
								frm.communicate({
									request_id : request_id,
			...
			function proxyRequest(frame, proxyData) {
				...
				if ( proxyData.pgProvider === "uplus" ) {
					jQuery.getScript(proxyData.custom.sdk, function() {
						...
						if (UserAgent.mobile() && (proxyData.target == "_top" || proxyData.target == "_self")) {
							frame.close();
						}
					});
					...
					jQuery.getScript(proxyData.custom.sdk, function() {
						...
						easypay_card_webpay(form, proxyData.custom.bridge, "_top", "0", "0", "submit", 30);
					});
					...
					jQuery.getScript(proxyData.custom.sdk, function() {
						...
						if (proxyData.custom.channel == 'mobile' && !proxyData.custom.popup) {
		...
		Frames.prototype.create = function(setting, callback) {
			...
			var key = setting.is_default ? DEFAULT_KEY : this._key(setting.provider, setting.pg_id, setting.type);
			if ( this.frames[key] )	remove.call(this, key);

			var that 	= this,
				$iframe = jQuery('<iframe src="about:blank" width="100%" height="100%" frameborder="0"/>').css({
					position: 'absolute',
					left: 0,
					right: 0,
					top: 0,
		...
		Frames.prototype.createCloseBtn = function() {
			...
			this.dialog.append(jQuery('<div class="imp-header"></div>').append(close));
		}

		Frames.prototype.find = function(provider_with_id, action_pre) {
			function match(f, provider, pg_id, action) {
				return f.provider === provider && (!pg_id || f.pg_id === pg_id) && f.type == action;
			}
			...
			if (this.frames[DEFAULT_KEY].type == action)	return this.frames[DEFAULT_KEY];
		...
		Frames.prototype.path = function(setting) {
			...
			var segment = setting.type === 'certification' ? '/certificates/ready/' : '/payments/ready/';
		...
		Frames.prototype.open = function(frm, data) {
			...
			this.dialog.show();
			this.dialog.removeClass().addClass('imp-dialog customizable').addClass(frm.type + '-' + frm.provider).addClass(uaClass);

			if ( !!data.popup && frm.type == 'certification' )	this.dialog.addClass("popup"); //다날 본인인증 popup 모드면 하얀 바탕 보여주지 않도록(TODO : Frames.communicate()가 visible iframe을 기반으로 처리하고 있어서 문제가 있음)
		...
		Frames.prototype.communicate = function(message) {
			for( var k in this.frames ) {
				...
				if ( $ifr.is(':visible') ) { //TODO : visible만으로 통신할 대상으로 판단하는 것은 위험하다
					var msg = {action:'communicate', data:message, from:'iamport-sdk', version:'1.2.0'},
						obj = JSON.stringify(msg); //받는 쪽에서 parse하므로 string이라도 stringify해서 넘김

					if ( UserAgent.ie() == 8 || UserAgent.ieCompatibilityMode() ) {
						(function(_ifr) {
							setTimeout(function() {_ifr[0].contentWindow.postMessage(obj, api_server)}, 0);
						})($ifr);
					} else {
		...
		var _XDM = {
				...
				jQuery(document).ready(function($) { //dom ready상태가 되기 전에 init이 호출된 경우도 있을 수 있다.
					...
					frm.load(function(pg_key) { //load될 때마다 계속 불려짐
						if ( reserved_request ) {
							...
							if ( f.key == pg_key ) {
								this.request(reserved_request.action, reserved_request.data, reserved_request.callback);
							}
						}
					});
				});
			...
			request : function(action, data, callback) {
				jQuery(document).ready(function($) { //dom ready상태가 되기 전에 호출된 경우도 있을 수 있다. ready handler를 차례대로 등록해 실행 순서를 보장
					try {
						var frm = getFrameInst();
						if ( !frm.user_code )	return alert('판매자 코드가 설정되지 않았습니다. IMP.init()함수를 먼저 호출하세요.');
							...
							defaultCallback = function(rsp) {
								if ( !rsp.success ) {
									if ( data.m_redirect_url ) {
										...
										location.href = encodeURI( Util.injectQuery(data.m_redirect_url, q) );
									} else {
										var q = {
											imp_uid : rsp.imp_uid,
											success : 'false',
										...
										location.href = encodeURI( Util.injectQuery(api_server + '/payments/fail', q) );
									}
								}
							};
						...
						if ( !frm.initialized() ) {
							return reserved_request = {action:action, data:data, callback:callback};
						}
              ...
              var error_msg = '등록되지 않은 PG모듈 정보입니다. 아임포트 관리자페이지에서 PG모듈 정보를 설정하신 후 다시 시도해주세요.';
              ...
              return; //[2019-09-25]find함수 내부에서 match되는 것을 찾는데 실패했을 때, default 를 반환하게 되는데 not null임을 강조하기 위해 type이 맞지 않는 것을 줄 수도 있다.
						...
						if ( f.loaded ) {
							...
							if ( action == 'certification' ) {
								if ( f.provider == 'danal' ) {
									...
									danalCert.open(data.request_id, data.merchant_uid, data.popup);
								} else if ( f.provider == 'inicis' ) {
									var inicisCert = INICISCERT.init(frm);
									inicisCert.open(data.request_id, data.merchant_uid, data.popup);
								} else if ( f.provider == 'inicis_unified' ) {
									var inicisUnifiedCert = INICISUNIFIEDCERT.init(frm);
									inicisUnifiedCert.open(data.request_id, data.merchant_uid, data.popup);
								}
							} else if ( action == 'zzim' ) {
								...
								if ( f.provider === 'payco' && !UserAgent.mobile() ) {
									...
									naverpay.open(data.request_id, data.merchant_uid, data.naverPopupMode, data.naverV2);
								} else if ( f.provider == 'paypal' ) {
									var paypalModal = PAYPALMODAL.init(frm);
									paypalModal.open(data.request_id, data.merchant_uid, data.popup);
									...
									settleFirm.open(data.request_id, data.merchant_uid, data.popup, f);
                  ...
                  if (!UserAgent.mobile()) {
							...
							var msg = {action:action, data:data, origin:location.href, from:'iamport-sdk', version:'1.2.0'};
							var obj = JSON.stringify(msg); //받는 쪽에서 parse하므로 string이라도 stringify해서 넘김

							if ( UserAgent.ie() == 8 || UserAgent.ieCompatibilityMode() ) {
								setTimeout(function() {f.iframe.contentWindow.postMessage(obj, api_server)}, 0);
							} else {
							...
							reserved_request = {action:action, data:data, callback:callback};
						}
					} catch(e) {
        ...
        jQuery(document).ready(function ($) {
          ...
          function checkParameters(props) {
            ...
            if (!userCode || !uid || !usecret) {
              return {
                isValid: false,
                errorMsg: '가맹점 식별코드(userCode), 구매자 아이디(uid), 구매자 시크릿(usecret)은 필수 입력입니다',
              };
            }
            // paymentData
            if (action !== 'manage') {
              ...
              if (!buyerEmail || !buyerTel) {
                return {
                  isValid: false,
                  errorMsg: '구매자 이메일(buyerEmail), 구매자 연락처(buyerTel)는 필수 입력입니다',
                };
              }
            }
          ...
          if (typeof callback == 'function') {
            sent_queue.push({ request_id:requestIdForPayport, callback:callback });
          }
          ...
          $(payportIframe).bind('load', function() {
            ...
            payportIframe.contentWindow.postMessage(payload, payportFrontDomain);
