...
const initSdk: InitSdkV1xFn = ({ window, api_server }) => {
  ...
  var NAVERCO = (function () {
    ...
    NaverCheckout.prototype.open = function (request_id, merchant_uid) {
      ...
      this.popup = window.open(
        "about:blank",
        "IMP_NAVER_CHECKOUT", /*, 'height=400,width=640,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes'*/
      );
      if (!this.popup) {
  ...
  var DANALCERT = (function () {
    ...
    DanalCert.prototype.open = function (request_id, merchant_uid, popupMode) {
      ...
      this.popup = window.open(
        "about:blank",
        this.targetName,
        "height=640,width=440,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes",
      );
    ...
    Frames.prototype.find = function (provider_with_id, action) {
      ...
      function match(f, provider, pg_id) {
        return f.provider === provider && (!pg_id || f.pg_id === pg_id) &&
          f.type == "payment"; //[2018-04-06] pg : danal 줬을 때 본인인증 창이 열리는 버그회피
      }
