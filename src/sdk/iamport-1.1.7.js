window.IMP || (function(window) {
	...
	var NAVERCO = (function() {
		...
		NaverCheckout.prototype.open = function(request_id, merchant_uid) {
			if ( UserAgent.mobile() )		return;

			this.popup = window.open('about:blank', "IMP_NAVER_CHECKOUT"/*, 'height=400,width=640,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes'*/);
			if ( !this.popup ) {
				alert("팝업차단을 해제해주셔야 결제창이 나타납니다.");
			}
		}
	...
	var DANALCERT = (function() {
		...
		DanalCert.prototype.open = function(request_id, merchant_uid, popupMode) {
			...
			this.popup = window.open('about:blank', this.targetName, 'height=640,width=440,location=no,status=yes,dependent=no,scrollbars=yes,resizable=yes');
	...
	var EXIMBAY = (function() {
		...
		EximbayPG.prototype.open = function(request_id, merchant_uid) {
	...
	var XDM = (function(window) {
		...
		Frames.prototype.find = function(provider_with_id, action) {

			//문자인증 임시코드
			if ( action === 'certification' ) {
				for( var k in this.frames ) {
					if ( this.frames[k].type === 'certification' ) 	return this.frames[k];
				}
			}

			function match(f, provider, pg_id) {
				return f.provider === provider && (!pg_id || f.pg_id === pg_id) && f.type == "payment"; //[2018-04-06] pg : danal 줬을 때 본인인증 창이 열리는 버그회피
			}
			...
			if ( match(f, provider, pg_id) )	return f;
		...
		Frames.prototype._key = function(provider, pg_id, action) {
			...
			if ( pg_id )		return provider + '.' + pg_id + '.' + action;

			return provider; //provider값이 inicis.INIpayTest와 같을 수도 있음
		}
		...
		Frames.prototype.communicate = function(message) {
			for( var k in this.frames ) {
				...
				if ( $ifr.is(':visible') ) { //TODO : visible만으로 통신할 대상으로 판단하는 것은 위험하다
					var msg = {action:'communicate', data:message, from:'iamport-sdk', version:'1.1.7'},
			...
			request : function(action, data, callback) {
				jQuery(document).ready(function($) { //dom ready상태가 되기 전에 호출된 경우도 있을 수 있다. ready handler를 차례대로 등록해 실행 순서를 보장
					try {
						...
						if ( action === 'certification' && action !== f.type )	return; //2016-12-01: action !== f.type인데, payments는 혹시 모르니 막지말자...
						...
						if ( f.loaded ) {
							...
							if ( action == 'certification' ) {
								...
								if ( f.provider === 'payco' && !UserAgent.mobile() ) {
									...
									naverco.open(data.request_id, data.merchant_uid);
									...
									eximbay.open(data.request_id, data.merchant_uid);
							...
							var msg = {action:action, data:data, origin:location.href, from:'iamport-sdk', version:'1.1.7'};
