window.IMP || (function(window) {
	...
	var SETTLEBANK = (function() {
		...
		SETTLEBANK.prototype.close = function(){
			if( this.popup){
				this.popup.close();
			}
		}
	...
	var Smartro = (function() {
		...
		Smartro.prototype.close = function() {
			if ( this.popup ) {
				this.popup.close();
			}
		}
		...
		var getFrameInst = (function() {
			...
			function init() { //jQuery.ready가 된 상태에서 호출되어야 함
				...
				function on_message(e) {
					...
					if ( origin !== api_server )	return false;
					...
					if ( action === 'kakao.dlp' ) { //mobile ios전용
						...
						return; //break;
					} else if ( action === 'settle.modal.close' ) {
						var settleModal = SETTLEBANK.instance();
						settleModal.close();
						...
						// 이니시스 통합인증은 팝업모드가 가능한데, 아래 로직은 리디렉션일때만 동작해야하므로 팝업인지 체크하는 로직을 추가적으로 적용
						if (UserAgent.mobile() && (!data.popup && (data.target == "_top" || data.target == "_self"))) {
							frm.close();
						}
						return; //break
					...
					} else if ( action === 'smartro.modal.close' ) {
